...
void CMD::__init() {
    __cmd_list[CMD_HELP]                = cmd_::help;
    __cmd_list[CMD_RELOAD_JSON]         = cmd_::reload_json;
    __cmd_list[CMD_RELOAD_PHRASES]      = cmd_::reload_phrases;
    __cmd_list[CMD_REINIT_JOYSTICK]     = cmd_::reinit_joystick;
    __cmd_list[CMD_REINIT_AUDIO]        = cmd_::reinit_audio;
    __cmd_list[CMD_GETMEM]              = cmd_::getmem;
    __cmd_list[CMD_GET_RADIO]           = cmd_::get_radio;
    __cmd_list[CMD_GET_WEATHER]         = cmd_::get_weather;
    __cmd_list[CMD_GET_POSITION]        = cmd_::get_position;
    __cmd_list[CMD_GET_RADIO_RANGE]     = cmd_::get_radio_range;
    __cmd_list[CMD_GET_RADIO_QUALITY]   = cmd_::get_radio_quality;
    __cmd_list[CMD_MONITOR]             = cmd_::monitor;
    __cmd_list[CMD_SHOW_KEYWORDS]       = cmd_::show_keywords;
    __cmd_list[CMD_HIDE_KEYWORDS]       = cmd_::hide_keywords;
    __cmd_list[CMD_SHOW_READBACK]       = cmd_::show_readback;
    __cmd_list[CMD_HIDE_READBACK]       = cmd_::hide_readback;
    __cmd_list[CMD_SHOW_LDISTANCE]      = cmd_::show_ldistance;
    __cmd_list[CMD_HIDE_LDISTANCE]      = cmd_::hide_ldistance;
    ...
    if (!cmdline.empty()) {
        ...
        if (__cmd_list.contains(cmd)) {
            ...
            case cmd_::help:
                ...
                X_OUTPUT::xprint(MSG_STYLE::HELP, CMD_GET_RADIO_RANGE, "Display radio depending on altitude and weather.");
                X_OUTPUT::xprint(MSG_STYLE::HELP, CMD_GET_RADIO_QUALITY, "Display radio and speaker noise volume.");
                break;

            case cmd_::get_radio_quality:
            ...
            case cmd_::show_keywords:
                SESSION::show_keywords = true;
                break;

            case cmd_::hide_keywords:
                SESSION::show_keywords = false;
                break;

            case cmd_::show_readback:
                SESSION::show_readback = true;
                break;

            case cmd_::hide_readback:
                SESSION::show_readback = false;
                break;

            case cmd_::show_ldistance:
                SESSION::show_ldistance = true;
                break;

            case cmd_::hide_ldistance:
                SESSION::show_ldistance = false;
                break;

            default:
                if (!SESSION::is_monitor) {
                    switch(__cmd_list[cmd]) {
                    case cmd_::reload_json:
                        //    init_json();
                        break;

                    case cmd_::reload_phrases:
                        //    load_phrase_file();
                        break;
